...
srer_mupolygon_raw <- read_gssurgo_mupolygon(gdb_path, aoi = srer_boundary_5070)
message("01: clipping MUPOLYGON to SRER boundary ...")
srer_mupolygon <- sf::st_make_valid(
  sf::st_intersection(srer_mupolygon_raw, srer_boundary_5070)
)
# Drop boundary attribute columns picked up during intersection;
# keep only the MUPOLYGON-side columns we care about.
srer_mupolygon <- srer_mupolygon[, "mukey", drop = FALSE]

poly_mukeys <- unique(as.character(srer_mupolygon$mukey))
message(sprintf(
  "01: %d MUPOLYGON features after AOI clip, %d unique mukeys",
  nrow(srer_mupolygon), length(poly_mukeys)
))

# ── Step 5: Rasterize MUPOLYGON to 10 m mukey grid ──────────────────────────
# The official NRCS MapunitRaster_10m is not used: the GDAL 3.8.4 OpenFileGDB
# driver in this environment does not expose FileGDB raster layers. The
# rasterized MUPOLYGON is functionally equivalent for analysis purposes.

message("01: rasterizing MUPOLYGON to 10 m mukey grid ...")
srer_raster <- rasterize_mupolygon(srer_mupolygon, resolution = 10)
srer_raster <- terra::mask(srer_raster, terra::vect(srer_boundary_5070))
...
write_provenance_sidecar(
  ...
  notes = "Vector polygons in EPSG:5070 (gSSURGO native CRS), clipped to the SRER station boundary using sf::st_intersection() + sf::st_make_valid(). Clipped (polygons) / masked (raster) to the SRER station boundary after rasterization. The pre-clip output covered ~44,000 ha — roughly 2x the boundary area — because MUPOLYGON features that intersect SRER extend beyond the boundary on average. Decision recorded 2026-05-15. Downstream analysis uses the intersection of polygon and raster mukey sets (PI decision 2026-05-14); mukeys present only in polygons are logged in the mismatch and unknowns logs."
)
message(sprintf("01: wrote %s", poly_path))

# 10b: Rasterized mukey grid
...
write_provenance_sidecar(
  ...
  input_sources = c(gssurgo_input_sources, list(
    layer      = "MUPOLYGON (rasterized)",
    output_crs = "EPSG:5070",
    resolution = "10m"
  )),
  notes = "Raster generated by terra::rasterize() of MUPOLYGON at 10 m resolution in EPSG:5070, then masked to the SRER station boundary using terra::mask(). The official NRCS MapunitRaster_10m was not used because the GDAL 3.8.4 OpenFileGDB driver in this environment does not expose FileGDB raster layers. The generated raster is functionally equivalent for analysis purposes — the cell values are the same mukeys placed on the same 10 m grid — but is not bitwise-identical to the NRCS raster. Clipped (polygons) / masked (raster) to the SRER station boundary after rasterization. The pre-clip output covered ~44,000 ha — roughly 2x the boundary area — because MUPOLYGON features that intersect SRER extend beyond the boundary on average. Decision recorded 2026-05-15."
)
